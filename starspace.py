...
class EmbeddingIntentClassifier(object):
    ...
    def __init__(self,
        ...
        self.num_hidden_layers_a=1
        self.hidden_layer_size_a=[250]
        self.num_hidden_layers_b=1
        self.hidden_layer_size_b=[250]
        self.batch_size=100
        self.epochs=50

        # embedding parameters
        self.embed_dim=20
        self.mu_pos=0.8  # should be 0.0 < ... < 1.0 for 'cosine'
        self.mu_neg=-0.4 # should be -1.0 < ... < 1.0 for 'cosine'
        self.similarity_type='cosine' # string 'cosine' or 'inner'
        self.num_neg=8
        self.use_max_sim_neg=True  # flag which loss function to use
        ...
        # (self.num_hidden_layers_a, self.hidden_layer_size_a) = self._check_hidden_layer_sizes(
    ...
    def _create_intent_dict(self,y):
        def quchong(li):
            new_li = []
            for i in li:
                if i not in new_li:
                    new_li.append(i)
            return new_li
        distinct_intents_dic = {intent: idx for idx, intent in enumerate(quchong(y))}
        return distinct_intents_dic

    def _create_intent_token_dict(self,y):
        """Create intent token dictionary"""
        distinct_tokens = set(y)
        return {token: idx for idx, token in enumerate(sorted(distinct_tokens))}

    def _create_encoded_intents(self,y):
        """Create matrix with intents encoded in rows as bag of words,
        if intent_tokenization_flag = False this is identity matrix"""
        intent_dict = self._create_intent_dict(y)

        if self.intent_tokenization_flag:
            intent_token_dict = self._create_intent_token_dict(y)
            ...
            for key, idx in intent_dict.items():
                ...
                encoded_all_intents[idx, intent_token_dict[key]] = 1
    ...
    def _prepare_data_for_training(self, training_data, y):
        """Prepare data for training"""
        intent_dict = self._create_intent_dict(y)
        X = training_data
        intents_for_X = np.array([intent_dict[e] for e in y])
        print(intents_for_X)
    ...
    def _tf_sim(self, a, b):
        ...
        if self.similarity_type == 'cosine':
            ...
            # b_norm = tf.sqrt(tf.reduce_sum(tf.square(b), axis=2))
        if self.similarity_type == 'cosine' or self.similarity_type == 'inner':
            # sim = tf.reduce_sum(tf.multiply(tf.expand_dims(a, 1), b), axis=-1) / (tf.expand_dims(a_norm, 1)* b_norm)
            sim = tf.reduce_sum(tf.expand_dims(a, 1) * b, -1)
            # similarity between intent embeddings
            sim_emb = tf.reduce_sum(b[:, 0:1, :] * b[:, 1:, :], -1)
            # sim_emb = tf.reduce_sum(tf.multiply(b_norm[:, 0:1, :], b_norm[:, 1:, :]), axis=-1) / (b_norm[:, 0:1, :]* b_norm[:, 1:, :])
            return sim, sim_emb
        else:
    ...
    def _tf_loss(self, sim, sim_emb):
        ...
        if self.use_max_sim_neg:
            ...
            loss = tf.reduce_mean(tf.maximum(0., self.mu_pos - sim[:, 0]) + tf.maximum(0., self.mu_neg + max_sim_neg))
        else:
    ...
    def _create_tf_graph(self, a_in, b_in, is_training):
        ...
        print(a_in) #Tensor("a:0", shape=(?, 460), dtype=float32)
        print(b_in) #Tensor("b:0", shape=(?, ?, 86), dtype=float32)
        ...
        b = self._create_tf_embed_nn(b_in, is_training,
                                     ...
                                     self.hidden_layer_size_b,
                                     name='b')
        print(a) #Tensor("embed_layer_a/BiasAdd:0", shape=(?, 28), dtype=float32)
        print(b) #Tensor("embed_layer_b/BiasAdd:0", shape=(?, ?, 28), dtype=float32)
    ...
    def _create_batch_b(self, batch_pos_b, intent_ids):
        ...
        for b in range(batch_pos_b.shape[0]):
            ...
            negative_indexes = [i for i in range(self.encoded_all_intents.shape[0])
                                if i != intent_ids[b]]
    ...
    def _train_tf(self, X, Y, helper_data,
        ...
        for ep in range(self.epochs):
            ...
            self._output_training_stat(X, intents_for_X, all_Y,sess, a_in, b_in,sim, is_training,ep, sess_out)
    ...
    def _output_training_stat(self,
        ...
        print("epoch {} / {}: loss {}, train accuracy : {:.3f}"
                    "".format((ep + 1), self.epochs,
                              sess_out.get('loss'), train_acc))

    def train(self, x, y,cfg=None, **kwargs):
        ...
        X, Y, helper_data = self._prepare_data_for_training(x, y)
        ...
        with self.graph.as_default():
            ...
            self._train_tf(X, Y, helper_data,sess, a_in, b_in, sim,loss, is_training, train_op)
    ...
    def process(self, message, **kwargs):
        ...
        if self.session is None:
            ...
            if intent_ids.size > 0:
                intent = {"name": self.inv_intent_dict[intent_ids[0]], "confidence": message_sim[0]}
    ...
    def load(cls,
        ...
        if model_dir:
            file_name = 'intent_classifier_tensorflow_embedding.ckpt'
